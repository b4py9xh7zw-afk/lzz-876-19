...
5. 成绩统计：个人成绩与管理端统计数据。
6. 断网续考保护：
   - 前端实时把**答案、题目状态（未答/已答/待复查）、本地修改时间**暂存到 `localStorage`，断网可继续答题；
   - 心跳保活，网络恢复后自动续考并补传草稿，计时以**服务端时间**为准；
   - 断网超过允许时长（默认 180 秒）自动转入“待监考处理”，教师可在“断网续考”工作台**批准延时**或**按暂存答案交卷**；
   - 后端通过设备指纹、标签页会话 ID、页面启动 ID、心跳缺口与客户端自报原因交叉判定，区分**真实断网 / 刷新页面 / 换设备登录**，只做风险分级，**不自动记为作弊**。
...
## 数据库说明
当前初始化后包含 12 张核心表（含用户、题目、试卷、考试记录、答案记录、答题草稿、考试会话事件等）。
